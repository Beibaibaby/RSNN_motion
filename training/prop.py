...
gt_high_contrast=np.load('large_high.npy')
#print(gt_high_contrast)
...
def experiment_train(cprime,Je,Jot,Jop):
        (J11, J12, J13, J14) = (Je, Jot, Jop, Jot)
        (J21, J22, J23, J24) = (Jot, Je, Jot, Jop)
        (J31, J32, J33, J34) = (Jop, Jot, Je, Jot)
        (J41, J42, J43, J44) = (Jot, Jop, Jot, Je)
...
def loss_function(result,gt):
    gt = gt.T
    gt_left=gt[0]
    gt_right=gt[1]
    result_left = np.zeros(gt_left.shape)
    result_right= np.zeros(gt_right.shape)
    for i in range(np.size(gt_left)):
        result_left[i]= result[0,i*100]
        result_right[i] = result[2, i * 100]
...
def simulate(cprime,gt,Je,Jot,Jop):
...
def compute_gradient(contrast_list,gt,Je,Jot,Jop,spacesize):
    ...
    loss_plus=[]
    loss_minus = []
    Je_temp = Je+spacesize
    Je_temp_2 = Je-spacesize
    gradient_Je=(simulate(cprime,gt,Je_temp,Jot,Jop)-simulate(cprime,gt,Je_temp_2,Jot,Jop))/(2*spacesize)
    Jot_temp = Jot+spacesize
    Jot_temp_2 = Jot-spacesize
    gradient_Jot=(simulate(cprime,gt,Je,Jot_temp,Jop)-simulate(cprime,gt,Je,Jot_temp_2,Jop))/(2*spacesize)
    Jop_temp = Jop+spacesize
    Jop_temp_2 = Jop-spacesize
    gradient_Jop=(simulate(cprime,gt,Je,Jot,Jop_temp)-simulate(cprime,gt,Je,Jot,Jop_temp_2))/(2*spacesize)


    """for i in range(spacesize):
        Je_temp = Je+i*0.0001
        parameter_list_pos.append(Je_temp)
        Je_temp_2 = Je - i * 0.001
        parameter_list_neg.append(Je_temp_2)
        loss_plus.append(simulate(cprime,gt,Je_temp,Jot,Jop))
        loss_minus.append(simulate(cprime,gt,Je_temp_2,Jot,Jop))"""


    #loss_minus.reverse()
    #parameter_list_neg.reverse()
    #parameter_list = parameter_list_neg+parameter_list_pos
    #loss_space=loss_minus + loss_plus
    #loss_space=np.asarray(loss_space)
    ...
    for cprime in [contrast]:
        if sum_plot == 0:
            ...
            for i in range(iter):
                ...
                result = np.asarray(result)
                np.save('./resultdata_lowcontrast_smallsize', result)
            ...
            # plt.show()
            plt.legend()
            plt.savefig('./allthree_' + str(cprime) + '.png')
            plt.show()
        elif sum_plot == 1:
            for i in range(iter):
                print(i)
